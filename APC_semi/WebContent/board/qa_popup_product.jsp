	...
	<div id="popup_container" align="center">
		...
		<div id="bottom">
			...
			<c:if test="${page > block }">
				...
				<a href="<%=request.getContextPath()%>/qa_product_search.do?page=1">
				<img src="e_image/btn_first.png" alt="btn_first">
				</a>
				<!-- 1페이지로 이동 (최초페이지로 이동)-->
				<a
					href="<%=request.getContextPath() %>/qa_product_search.do?page=${startBlock-1}">
			...
			<c:forEach begin="${startBlock }" end="${endBlock }" var="i">
				<c:if test="${i == page }">
					...
					<b><a
						href="<%=request.getContextPath()%>/qa_product_search.do?page=${i}">[${i }]</a></b>
				</c:if>
				<c:if test="${i != page }">
					<!-- startBlock != page, 현재재페이지가 아니라면 -->
					<a href="<%=request.getContextPath()%>/qa_product_search.do?page=${i}">[${i }]</a>
				</c:if>
			</c:forEach>

			<c:if test="${endBlock < allPage }">
				<!-- endBlock이 전체페이지 수보다 작은경우, 예를 들어 endBlock이 6페이지인경우 -->
				<a href="qa_product_search.do?page=${endBlock + 1 }">
				<img src="e_image/btn_next.png" alt="btn_next">
				</a>
				<!-- 7페이지로 이동(다음 페이지로 이동) -->
				<a href="qa_product_search.do?page=${allPage }">
