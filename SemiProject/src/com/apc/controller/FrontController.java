...
public class FrontController extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		 ...
		 * - 보통은 파일에 저장하여 사용을 함. 파일 이름은 *.properties 이름으로 끝나게 함.
		...
		FileInputStream fis =
			new FileInputStream("C:\\Users\\fontk\\git\\KH_SemiProjeckt_team3\\SemiProject\\src\\com\\apc\\controller\\mapping.properties");
		...
		if(value.substring(0, 7).equals("execute")) {
			 ...
			 *   인터페이스를 사용하는 것임.
			 *   
			 * - Class.forName(class 이름)은 파라미터로 받은 class 이름에 해당하는 클래스를
			 *   로딩한 후에, 그 클래스에 해당하는 인스턴스를 리턴을 해 줌.
			 *   newInstance() 메서드는 로딩할 클래스의 객체를 생성하는 메서드이고.
			 *   이렇게 생성된 객체를 동적 객체생성이라고 함.
			 */ 
			...
			try {
				...
				// 동적으로 객체를 생성하는 방법
				// Action 인터페이스 타입으로 상속을 받아 사용하므로 형변환
